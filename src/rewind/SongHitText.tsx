...
export default function SongHitText({ songHitsOnDays }: SongHitTextProps) {
  ...
  const scale = spring({ fps, frame, delay: 90, durationInFrames: 60 })
  const scale2 = interpolate(scale, [0, 1], [500, 0], { extrapolateLeft: "clamp" })

  return <Animated in={30} className="flex justify-center " animations={[Fade({ initial: 0, to: 1, start: 30 })]}>
    <p style={{ transform: `translateY(${scale2}px)` }} className="text-6xl font-bold text-center w-4/5 leading-snug">You've looked at {interpolatedCount.toFixed(0)} song entries on {interpolatedDays.toFixed(0)} days </p>
  </Animated>
}